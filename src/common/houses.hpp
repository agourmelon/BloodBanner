...
public:
    ...
    explicit HouseCardHand(
        std::array<HouseCard, HOUSE_CARDS_PER_PLAYER> &&
            cards // NOLINT(cppcoreguidelines-rvalue-reference-param-not-moved)
    )
        : availables_{
              std::make_move_iterator(std::begin(cards)),
    ...
    [[nodiscard]] const std::vector<HouseCard> & available() const { return availables_; }
    [[nodiscard]] const std::vector<HouseCard> & used() const { return used_; }
...
struct House {
    ...
    House(std::string_view n, HouseCardHand && c)
        : name{n}
        , cards{std::move(c)} {}
